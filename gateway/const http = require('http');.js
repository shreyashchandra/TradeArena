...
// PostgreSQL Connection Pool (Configured for IPv4 Supavisor)
const pool = new Pool({
...
// Catch silent background database crashes
pool.on('error', (err) => {
...
waitForDatabase()
  .then(() => {
    server.listen(PORT, () => {
      console.log(`tradearena gateway listening on port ${PORT}`);
      connectMarketDataStream();
    });
  })
  .catch((error) => {
    console.error('FATAL ERROR: Failed to connect to database on startup.');
    console.error(error);
    process.exit(1);
  });